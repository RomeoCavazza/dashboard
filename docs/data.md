...
- `name` → `"Ligue 1"`
- `emblem` → URL PNG du logo (utilisable directement dans le header)
- `currentSeason.currentMatchday` → `22` (journée en cours)
- `currentSeason.startDate` → `"2025-08-17"` → saison **2025**
- `currentSeason.id` → `2420` (à utiliser comme filtre `season=2025` sur les autres endpoints)

---

## 1. Vue d'ensemble des endpoints utilisés
...
## 2. Structure logique des datasets nécessaires
...
## 4. Agrégations simples nécessaires
...
## 5. Stratégie d'optimisation des appels API
...
## 7. Flux de chargement recommandé
...
Via Postman (`postman/postman_collection.customization`) :
